...
public class deltaRecordCal extends PApplet {
    ...
    private static final int[] DELTALIST = {0, 4, 8, 16, 32, 64, 128};
    private static final double[] RATELIST = {0.1, 0.05, 0.01, 0.005, 0.001, 0.0005, 0.0001, 0.00005, 0.00001};

    // [][] for cellList (HashSet, i.e. VFGS result R).
    private static List<HashSet<Integer>[]> deltaCellList = new ArrayList<>();
    private static String dataPath = "C:\\Users\\Administrator\\Desktop\\zhengxin\\vfgs_origin_data\\shenzhen\\sz_score.txt";
...
//                System.out.println(RATELIST[n] + ": " + (t2 - t0) + "ms");
                System.out.println("alpha," + RATELIST[n] + ",delta," + DELTA + ",time,"
                        + (t2 - t0));
            }
...
//            System.out.println("begin write delta " + DELTA);
            ...
            for (int k = 0; k < cellList.length; k++) {
                BufferedWriter writer = new BufferedWriter(new FileWriter(String.format("data" +
                        "/sz_week_0930/vfgs_%d.txt", DELTA)));   // !!!
